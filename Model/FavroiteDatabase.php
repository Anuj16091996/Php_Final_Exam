...
class FavroiteDatabase
{
    ...
    public static function UserLike($UserID)
    {
        $stmt = new DatabaseConnection();
        $value = $stmt->GetConnect();
        $checkQuery = "select favorite.ParnerId, images.ImageName,customer.FirstName,customer.LastName from favorite join customer on favorite.ParnerId = customer.CustomerID  join images on customer.CustomerID = images.CustomerID where favorite.CustomerID='$UserID' and favorite.CustomerID!='null'";
        $result = $value->prepare($checkQuery);
        $result->execute();
        $result->setFetchMode(PDO::FETCH_ASSOC);
        return $result->fetchAll();
