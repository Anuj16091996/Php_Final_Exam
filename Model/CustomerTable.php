...
class CustomerTable
{
    ...
    public static function GetWholeTableDetails($userSex, $UserID)
    {
        $stmt = new DatabaseConnection();
        $value = $stmt->GetConnect();
        $query = "SELECT customer.FirstName, customer.LastName,customer.CustomerID,images.ImageName from customer join Images on customer.CustomerID=images.CustomerID WHERE customer.Sex='$userSex' and customer.CustomerID!='$UserID'";
        $result = $value->prepare($query);
        $result->execute();
        $result->setFetchMode(PDO::FETCH_ASSOC);
        return $result->fetchAll();
